...
if [[ "$dry_run" == 1 ]]; then
  ...
  echo "Would update Cargo.toml and Cargo.lock if the version changes."
  echo "Would commit: chore: release $tag (only when files change)"
  echo "Would tag: $tag"
  echo "Would push the default branch and tag, then create a GitHub release."
  exit 0
fi
